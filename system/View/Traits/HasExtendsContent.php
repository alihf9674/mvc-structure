...
trait HasExtendsContent
{
    ...
    private function initialYields($yield_name)
    {
        $string = $this->content;
        $startWord = "@section('" . $yield_name . "')";
        $endWord = "@endsection";
        $startPos = strpos($string, $startWord);
        if ($startPos === false)
            return $this->extendsContent = str_replace("@yield('$yield_name')", "", $this->extendsContent);
        ...
        return $this->extendsContent = str_replace("@yield('$yield_name')", $sectionContent, $this->extendsContent);
    }
}
